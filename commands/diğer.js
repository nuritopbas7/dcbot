...
message.channel.send(new Discord.MessageEmbed().setColor('#000001').setDescription(`\`\`\`Lykia Diğer Kullanabileceğin Komutlar Aşağıda Yer Almakta.\`\`\`
...
**Lykia bağlantılarını gönderir**

📋 \`${client.ayarlar.prefix}kurallar\`
**Lykia hazır tema ile kurallarınızı yerleştirir**

🌟 \`${client.ayarlar.prefix}yavaş-mod\`
**Lykia üyelerinizin belirlediğiniz sürede bir mesaj yazmasını sağlar**

\`${client.ayarlar.prefix}emote\`
**Emoji URL veya Emoji girerek eğlenceli emojinin çizilimini atar**

🐝 \`${client.ayarlar.prefix}emoji\`
**Lykia sunucunda ki emoji ismini yaz sana indirme bağlantısı verir**
...
**Lykia Telefon kullananlar için Rol Renk değiştirme**

🗑 \`${client.ayarlar.prefix}sil [100]\`
**Lykia bot kanaldaki mesajları siler**

\`${client.ayarlar.prefix}sunucudan-ayrıl\`
**Lykia ile kurmuş olduğunuz ayarlarınızı sıfırlayarak sunucundan çıkar**`).setThumbnail('https://media.giphy.com/media/3qXi5eIvcgCmTDObOU/giphy.gif'));
